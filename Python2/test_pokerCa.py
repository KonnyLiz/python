...
prro=("Todos Diferentes","Un Par","Dos Pares","Una Tercia","Full","Poker","Quintilla")
...
resul2 = 0.0
for p in range(7):
	resul.append(str(float(frecuenciaEsperada[p]) * 30))
	resul2 = resul2 + float(resul[p])

resul2 = ((resul2-30)**2)/resul2
print resul2

for i in range(7):
    h = [str(tipo[i]), str(juego[i]), str(esperado[i]), str(resultados[i])]
    listatabla.append(h)
    pass

print tabulate(listatabla, headers=[" TR "," FO "," FE "," r"])
