...
class MyDialog1 ( wx.Dialog ):
    ...
    def __init__( self, parent ):
        ...
        self.txt2 = wx.TextCtrl( self, wx.ID_ANY, wx.EmptyString, wx.DefaultPosition, wx.Size( 250,-1 ), 0 )
        ...
        fgSizer2.Add( self.btn_buscar_img, 0, wx.ALL, 5 )
    ...
    def btnAceptar( self, event ):
        ...
        if ( self.lbl1.GetLabel()=='INSERTAR'):
            ...
            self.id_cat = self.cbCat.GetSelection()
            with open(self.ruta) as img:
                self.imagen= img.read()
            sql = "INSERT INTO producto(barcode, id_categoria, descripcion, exento, costo, precio, fecha_ingreso, inactivo, imagen, tiene_imagen) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
            valores=(self.barcode,self.id_cat,self.descripcion,self.exento,self.costo,self.precio,fecha, self.inactivo,self.imagen,'1')
            typesql='I'
            query=self.db.query(sql,valores,typesql)
            self.dial.ShowModal()
            self.frm_padre.Cargar()
            
        if ( self.lbl1.GetLabel()=='ELIMINAR'):
            self.id_producto = str(self.txt1.GetValue())
            sql="DELETE FROM producto WHERE id_producto='%s'"%(self.id_producto)
            typesql='D'
...
###########################################################################
## Class MyDialog2 para boton ver
###########################################################################
